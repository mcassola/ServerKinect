...
using Size = ServerKinect.Shape.Size;
...
namespace ServerKinect.HandTracking.Visual
{
    public class HandLayer : LayerBase
    {
        ...
        private HandCollection currentHandColletion;
        ...
        {
            currentHandColletion = handData;
            this.OnRequestRefresh();
        }

        public HandCollection getCurrentHandCollection() { return currentHandColletion; }
        ...
        private void PaintFingerPoint(Graphics g, FingerPoint point)
        {
            ...
            if (!ServerKinect.Shape.Point.IsZero(point.BaseLeft) && !ServerKinect.Shape.Point.IsZero(point.BaseRight))
            {
                var baseCenter = ServerKinect.Shape.Point.Center(point.BaseLeft, point.BaseRight);
        ...
        {
            string handString = "";
            foreach (HandData hand in currentHandColletion.Hands)
            {
                //HAND ID AND FINGER COUNT
                handString += "/" + hand.Id + "#" + hand.FingerCount + "#";
               //FINGER POSITION
                foreach (FingerPoint finger in hand.FingerPoints)
                {
                    handString += finger.ToString() + "#";
                }
                //PALM POSITION AND HAND VOLUME
                handString += hand.PalmPoint.ToString() + "#" + hand.Volume.ToString();
